...
5. Check the stopping rules, in this order. **(a)** If $\mathcal{F}$ is empty, stop with **no MTD** (every dose is too toxic). **(b)** Otherwise, if the current dose is admissible, its $\pi^{T} \ge p^\ast$, its $\pi^{T}$ is the largest among the doses treated so far, and at least $n_{\min}$ patients have been treated at it, stop and declare the current dose as the **MTD**. **(c)** Otherwise, if the total enrolled has reached $N_{\max}$, stop with **no MTD** (inconclusive).
